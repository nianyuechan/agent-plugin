...
class AgentCore {
  ...
  /**
   * 执行 agent 模式对话
   * 对齐 Hermes: run_conversation(user_message, system_message, conversation_history)
   */
  async run(userId, event, userMessage, personality) {
    if (this._running.get(userId)) {
      return "⏳ Agent 正在处理中，请稍候..."
    }

    this._running.set(userId, true)
    ...
    try {
      ...
      this._running.set(userId, false)
    }
  }
   ...
   * 快速对话模式（无工具调用，直接回答）
   */
  async quickChat(userId, userMessage, personality) {
    contextManager.initSession(userId)
    await scanSkills()
  ...
  async _agentLoop(userId, event, systemPrompt) {
    ...
    for (let round = 0; round < MAX_ROUNDS; round++) {
      ...
      for (const toolCall of toolCalls) {
        ...
        const result = await toolRegistry.execute(fnName, fnArgs, { event, userId })
    ...
    // 循环结束，自动刷新记忆
    await memoryManager.flush()
    return finalContent || "Agent 执行完毕"
  }
